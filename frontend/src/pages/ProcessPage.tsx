...
export default function ProcessPage() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (response.status === 200 && data.signal === 'processing_success') {
        const resultData = {
          processed_files: data.processed_files,
          inserted_chunks: data.inserted_chunks,
        };
        ...
        if (data.signal === 'not_found_files') {
            toast.error('No documents found to process. Please upload files first.', { id: toastId });
        } else {
            toast.error(data.detail || data.signal || 'An unknown error occurred.', { id: toastId });
        }
      }
    } catch (error: any) {
